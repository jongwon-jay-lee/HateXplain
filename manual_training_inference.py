...
def train_model(params, device):
    ...
    if params['auto_weights']:
        y_test = [triple[2] for triple in test]
        #         print(y_test)
        encoder = LabelEncoder()
        encoder.classes_ = np.load(params['class_names'], allow_pickle=True)
        unique_labels = np.unique(y_test).tolist()
        balanced_class_weight = class_weight.compute_class_weight(
            'balanced', np.unique(y_test), y_test).astype('float32')
        weight_list = []
        for curr_class in encoder.classes_:
            if curr_class in unique_labels:
                b_idx = unique_labels.index(curr_class)
                weight_list.append(balanced_class_weight[b_idx])
            else:
                weight_list.append(1.0)
        # params['weights'] = class_weight.compute_class_weight('balanced', np.unique(y_test), y_test).astype('float32')
        params['weights'] = weight_list
    ...
    print('labels weight: {}'.format(params['weights']))
...
dict_data_folder = {
    ...
    '3': {'data_file': 'Data/sample_dataset.json', 'class_label': 'Data/classes.npy'}
}
