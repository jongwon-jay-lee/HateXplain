...
# This function is used to aggregate the attentions vectors.
# This has a lot of options refer to the parameters explanation for understanding each parameter.
def aggregate_attention(at_mask, final_label, params):
    ...
    if final_label in ['normal', 'non-toxic']:
        at_mask_fin = [1 / len(at_mask[0]) for x in at_mask[0]]
    else:
        at_mask_fin = at_mask
        # Else it will choose one of the options
        # 1. Variance is added.
        # 2. Mean is calculated.
        # 3. Finally, the vector is normalized.
