...
class SC_weighted_BERT(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        # output_hidden_states = False, output_attentions = True -> outputs[2] ==> attentions (B, # heads, L, L)
        ...
        if labels is not None:
            ...
            if self.train_att:
                ...
                loss_att = 0
                attention_output = outputs[1]   # (B, # head, L, L)
                for i in range(self.num_sv_heads):
                    # Get the i-th attention weights from most-upper trf layer
                    # at zero index (= from [CLS] to other tokens)
                    attention_weights = attention_output[self.sv_layer][:, i, 0, :]
